...
hentKnap.addEventListener('click', () => {
    ...
    const sideMargin = Math.round(printW * 0.06);           // hvid kant om siderne
    const fotoStørrelse = printW - sideMargin * 2;          // det firkantede foto
    const bundStribe = Math.round(fotoStørrelse * 0.20);    // skrive-plads = 20% af fotoets højde
    const kortHøjde = sideMargin + fotoStørrelse + bundStribe;
    const topPolstring = Math.round((printH - kortHøjde) / 2); // centrerer kortet lodret på papiret
    const fotoTop = topPolstring + sideMargin;
    ...
    fctx.drawImage(fotoCanvas, sideMargin, fotoTop);
